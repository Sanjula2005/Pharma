...
def combine_summaries(agent_outputs):
    ...
    for output in agent_outputs:
        ...
        if isinstance(output, dict):
            title = output.get("title", "Section")
            content = output.get("content", "")

            final_report.append(f"\n\n{title}\n{'-' * len(title)}")

            # Clinical trials special formatting
            if isinstance(content, dict) and "studies" in content:
                trials = content["studies"]
                if not trials:
                    final_report.append("No active or completed trials found.")
                else:
                ...
                final_report.append(str(content))
